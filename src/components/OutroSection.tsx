    ...
    <section ref={sectionRef} className="min-h-screen flex flex-col items-center justify-center px-6 py-24 relative">
      ...
      <div className="max-w-2xl text-center space-y-8">
        {lines.map((line, index) => (
          <motion.p
            key={index}
            className="font-elegant text-2xl sm:text-3xl md:text-4xl lg:text-5xl text-[#FFF5E1] leading-relaxed"
      ...
      >
        ...
        />
        <p className="text-[#F8C8DC] text-lg tracking-wider font-light">
          With love, always.
        </p>
      </motion.div>
      ...
      <motion.button
        ...
        onClick={onReplay}
      >
        <span className="relative z-10 px-8 py-4 block text-[#FFF5E1] text-sm tracking-[0.2em] uppercase font-light transition-colors duration-500 group-hover:text-[#D4AF37]">
          Replay the experience
        </span>
        
        {/* Button border */}
        <span className="absolute inset-0 border border-[#D4AF37] opacity-50 rounded-full transition-all duration-500 group-hover:opacity-100 group-hover:scale-105" />
        
        {/* Glow effect on hover */}
        <span className="absolute inset-0 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-[#D4AF37]/10 blur-xl" />
      </motion.button>
      ...
      >
        {[...Array(3)].map((_, i) => (
          <motion.span
            key={i}
            className="w-1.5 h-1.5 rounded-full bg-[#F8C8DC]"
            animate={{
              opacity: [0.3, 1, 0.3],
              scale: [1, 1.2, 1],
            }}
